...
teeth = 30;			// Number of teeth, standard Mendel T5 belt = 8, gives Outside Diameter of 11.88mm
...
motor_shaft = 6.4+play;	// NEMA23 motor shaft exact diameter = 6.4
...
{    
    union()
    {
        for(h=[0,10-1.5])
        translate([0,0,h])
            cylinder(h=1.5,r=21/2);

        translate([0,0,9])
        {
            cylinder(h=1+10,r=21/2);
        }
    }
    
    translate([0,0,-1])
        cylinder(h=22,r=motor_shaft/2);
    ...
    {
        rotate([0,0,180/6])
            cylinder(h=M3_nut_h+play,r=M3_nut_d/2+play,$fn=6);
        translate([-M3_nut_w/2,0,0])
            cube([M3_nut_w,10,M3_nut_h+play]);
        translate([0,0,-5])
            cylinder(h=10,r=3/2+play);
    }
}
